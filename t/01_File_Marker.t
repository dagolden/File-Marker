...
ok( $obj = File::Marker->new( $filename, "<" ), 
    "opening a data file with 'new'" 
);
...
ok( $obj->set_marker( "line3" ),
    "setting a marker for line 3"
);
...
ok( $obj->goto_marker( "line3" ),
    "jumping back to the marker for line 3"
);
...
is( scalar <$obj>, $expected_line,
    "reading line 3 again"
);

eval { $obj->set_marker('LAST') };
like( $@, qr/LAST/,
    "got error trying to set 'LAST'"
);
...
is_deeply( [ sort $obj->markers() ], \@expected_markers,
    "got correct list of existing markers"
);
...
ok( $obj->open( $filename, "<" ),
    "reopening test data file"
);

eval { $obj->goto_marker( "line3" ) };
like( $@, qr/line3/,
    "reopening clears tags (got error jumping to line 3 tag)"
);

is( scalar <$obj>, $line1,
    "reading line 1 again"
);
...
like( $@, qr/closed/,
    "got error trying to set marker for closed filehandle"
);
